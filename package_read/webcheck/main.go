...
func main(){
	...
	if len(os.Args)==1||os.Args[1]=="-h"{
		...
		fmt.Println("Output format:write to url.log\n")
		os.Exit(1)
	}
	//validate url
	...
	for {
		...
		log.Printf("Online. %s http-status:%d\n",u.String(),resp.StatusCode)  //write to .logfile
		fmt.Printf("Online. %s http-status:%d\n",u.String(),resp.StatusCode)  //Display terminal
